...
public class DebugDrawPhysics2D : MonoBehaviour {
	...
	void Start () {
		CreateLineMaterial ();
		boxColliders2D = Resources.FindObjectsOfTypeAll<BoxCollider2D> ();
		polygonColliders2D = Resources.FindObjectsOfTypeAll<PolygonCollider2D> ();
		circleColliders2D = Resources.FindObjectsOfTypeAll<CircleCollider2D> ();
	...
	void Update () {
		...
		for (int i = 0; i < boxColliders2D.Length; i++) {
			var c = boxColliders2D [i];
			boxPointList [i] = GetBoxPoints (c);
		}

		circlePointList = new Vector3[circleColliders2D.Length][];
		for (int i = 0; i < circleColliders2D.Length; i++) {
			var c = circleColliders2D [i];
			circlePointList [i] = GetCircleColliderPoints (c, CIRCLE_COLLIDER_SEGMENTS);
		}
